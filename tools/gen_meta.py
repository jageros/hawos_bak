...
def write_meta_file(filename, rpc_info):
    ...
    f = open("protos/meta/" + filename + ".go", "w")
    f.write(data)
    f.close()
...
"""
    ...
    for msgid in all_msgid:
        register_data += "    RegisterMeta(&%s_Meta{})\n" % msgid

    data += register_data
    data += """
}
"""
    f = open("protos/meta/meta_register.go", "w")
    f.write(data)
    f.close()
...
def fuck():
    all_msgid = parse_dir("protos/pbdef/")
    # all_msgid.extend(parse_dir("protos/pbsrv/"))
